...
namespace :db do
  ...
  task :bootstrap  => :environment do
    ...
    Dir.glob(File.join(ReviewsExtension.root, "db", 'sample', '*.{yml,csv}')).each do |fixture_file|
      Fixtures.create_fixtures("#{ReviewsExtension.root}/db/sample", File.basename(fixture_file, '.*'))
    end
  end
end
